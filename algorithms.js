...
var greyScale = async function (name, imagePath, secondImagePath) {
    try {
        ...
        const compare = await compareWithSecondAlgorithm(name, testPath1, testPath2);
...
var blur = async function (name, parameters, imagePath, secondImagePath) {
    ...
    await (await Jimp.read(imagePath)).blur(param.r).writeAsync(testPath1);
    await (await Jimp.read(secondImagePath)).blur(param.r).writeAsync(testPath2);
...
var gaussianBlur = async function (name, parameters, imagePath, secondImagePath) {
    const param = JSON.parse(parameters);
    await (await Jimp.read(imagePath)).gaussian(param.r).writeAsync(testPath1);
    await (await Jimp.read(secondImagePath)).gaussian(param.r).writeAsync(testPath2);
...
var compareWithSecondAlgorithm = async function (name, imagePath, secondImagePath) {
